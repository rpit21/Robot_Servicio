"""Deteccion de Colores, Forma, tracking, comandos, envio serial"""
...
def dibujar (mask,color):
    ...
    for c in contornos:  # se itera cada uno de los contornos encontrados 
        ...
        if area>=50 and len(approx) == 3:    # se filtran los contornos para quedarnos con aquellos que tienen un area mayor de 50 reduciendo el ruido y que la figura tenga 3 vertices
            ...
            cv2.drawContours(frame, [approx], 0, color, 3) # coloco el nuevo contorno en el frame (imagen original)
...
def Marca (colormarca):
    ...
    while(cap.isOpened()):    # Hacemos un bucle acorde a cuando la camara este habilitada
        ...
        if ret==True:   # Si una imagen (ret) es obtenida (true) 
            ...
            Mask=cv2.erode(Mask, None, iterations=1) # Elimina el ruido blanco de la mascara (para mayor erode aumentar 'iterations')
