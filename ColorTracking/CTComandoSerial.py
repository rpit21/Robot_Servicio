...
def dibujar (mask,color):
    ...
    for c in contornos:  # se itera cada uno de los contornos encontrados 
        ...
        if area>=50:    # se filtran los contornos para quedarnos con aquellos que tienen un area mayor de 1500 reduciendo el ruido
...
#Funcion para capturar el video de la camara, transformar HSV, y buscar cada una de las mascaras de acuerdo al color pasado en paramtro
def Marca (colormarca):
    ...
    while(cap.isOpened()):    # Hacemos un bucle acorde a cuando la camara este habilitada
        ...
        if ret==True:   # Si una imagen (ret) es obtenida (true) 
            ...
            envioSerialDatos(DatosMask) # Llama funcion para enviar la lista 'DatosMask'  por serial
